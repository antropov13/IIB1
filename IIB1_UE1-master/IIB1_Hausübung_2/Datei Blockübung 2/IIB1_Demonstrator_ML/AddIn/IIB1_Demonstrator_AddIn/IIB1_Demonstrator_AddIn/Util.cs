...
namespace IIB1_Demonstrator_AddIn
{
    public class Util
    {
        ...
        //Fensterflächengrenzwert
        private static void aendereNeueProperties (BindingList<Raum> raeume)
        {

        }

        public static void platziereLampen(BindingList<Raum> raeume)
        {
            foreach (Raum r in raeume)
                if (r.grenzwertUnterschritten())
                    platziereLampeInRaum(r);            
        }

        private static void platziereLampeInRaum(Raum r)
        {
                ...
                using(Transaction trans = new Transaction(doc))
                {
                    if (trans.Start("PlaceFamily") == TransactionStatus.Started)
                    {
                        FamilyInstance fi = doc.Create.NewFamilyInstance(locR, 
                            GetFamilySymbolByName(BuiltInCategory.OST_LightingFixtures, "F - 1850 x 450")
